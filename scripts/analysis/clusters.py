...
def main():
    ...
    quantized, ti_labels = cluster(test_images, km, a=a, b=b)
    show_images(quantized, ti_names,
                suptitle=f"Quantized Images by KM-centroids in [{a},{b}].")

    kept_cluster = keep_cluster(quantized, ti_labels, targets=clusters)
    show_images(kept_cluster, ti_names, suptitle=f"Cluster {clusters}")

    kept_cluster = keep_cluster(test_images, ti_labels, targets=clusters)
    show_images(kept_cluster, ti_names, suptitle=f"Original values of Cluster {clusters}")
