...
def _create_random_images(size: tuple[int, int], n_images: int,\
...
def _create_data(images, noise_level:float, fakeimages_proportion: float = 1.0)\
    -> tuple[list[np.ndarray], list[int]]:
    
    # crear imagenes aleatorias
    n_fakeimages = np.round(fakeimages_proportion*len(images))
    fake_images = _create_random_images(images[0].size, n_fakeimages,\
                                        noise_level)
...
def apply_lr(images: list[np.ndarray], show_generated: bool=False):
    ...
    # crear datos nuevos
    real_and_fake_images, targets = _create_data(images, noise_level,
                                                 fakeimages_proportion)
